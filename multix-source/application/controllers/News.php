...
class News extends CI_Controller {
	...
	public function view($slug=FALSE, $id=0)
	{
		if( !isset($id) || !is_numeric($id) || !isset($slug) ) {
			redirect(base_url());
		}

		$tot = $this->Model_news->news_check($id,$slug);
		if(!$tot) {
			redirect(base_url());
		}
	...
	public function xem($id=0, $slug=FALSE)
	{
		if(!isset($slug) ) {
			redirect(base_url());
		}
		...
		$data['news'] = $this->Model_news->all_news();
		$data['news_detail'] = $this->Model_news->news_detail_with_categoryz($slug);
		//$data['news_detail_category'] = $this->Model_news->news_detail_with_category($id);
		$data['category'] = $this->Model_news->get_category_name_by_id($data['news_detail']['category_id']);
		$data['category_name'] = $data['category']['category_name'];
		$data['id'] = $id;
